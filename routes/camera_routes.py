...
def create_camera(camera: schemas.CameraBase, db: Session = Depends(get_db)):
...
def update_camera(camera_id: int, camera: schemas.CameraBase, db: Session = Depends(get_db)):
    db_camera = camera_crud.get_camera(db, camera_id=camera_id)
    if db_camera is None:
        raise HTTPException(status_code=404, detail="Camera not found")
